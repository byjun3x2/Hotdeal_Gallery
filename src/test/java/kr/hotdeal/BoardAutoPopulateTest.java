...
public class BoardAutoPopulateTest {
	...
	private static final String[] CATEGORIES = { "먹거리", "sw/게임", "pc제품", "가전제품", "생활용품", "의류", "세일정보", "화장품", "모바일/상품권",
			"해외핫딜", "기타" };

	private static final String[] SAMPLE_COMMENTS = { "좋은 정보 감사합니다!", "이거 진짜 싸네요!", "구매했습니다~", "정보 공유 감사해요.",
			"와... 이 가격 실화인가요?", "배송비가 좀 아쉽네요.", "품절이네요 ㅠㅠ", "추천드립니다!", "비추천 이유가 뭔가요?", "이거 사도 될까요?" };
	...
	public void 게시판_자동_채우기_테스트() {
		...
		for (HotdealVO vo : hotdealList) {
			...
			System.out.println("[" + loaded.getId() + "] " + loaded.getTitle() + " / 추천: " + loaded.getLikes()
					+ " / 비추천: " + loaded.getDislikes());
		}
	}
}
